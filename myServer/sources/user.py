...
class user:
    ...
    def set_lock_flag(self, security_key, all, lock_flag):
    ...
    def set_place_station_id(self, station_id, place_id, security_key):
        db = MySQLdb.connect("localhost", "root", "myBP", "myBP_DB")
        cursor = db.cursor()
        
        try:
            update_sql = "UPDATE users SET  place_id='"+str(place_id)+"', station_id = '"+str(station_id)+"' WHERE username_code='"+security_key[0:25]+"' and pwd_code='"+security_key[25:50]+"';"
            print update_sql
            cursor.execute(update_sql)
            db.commit()
    ...
    def connection_stationDB(self, station_id, place_id, security_keyFromApp, registration_id, toDo):
        db = MySQLdb.connect("localhost", "root", "myBP", "myBP_DB")
        cursor = db.cursor()
        parking_data={}
            ...
            if ((security_key==security_keyFromApp and status==1 and ras_flag == 1)  or (security_key == 'None' and status == 0)):  
                ...
                if(toDo==1):
                    ...
                    try:
                        ...
                        print "station NOT updated [connection_stationDB()]"
            elif (((security_key == 'None' and status == 1) or (security_key==security_keyFromApp and status==1)) and ras_flag == 0) or (ras_flag==1 and security_key == 'None' and status == 1):
                ...
                if(toDo==1):
                    ...
                    parking_data['place_id']   = place_id
                    parking_data['lock']       = 0
            elif(security_key == security_keyFromApp and ras_flag==0): 
                parking_data['station_id'] = -1
                parking_data['place_id']   = -1
                parking_data['status']     =  0
                parking_data['lock']       = -1 
